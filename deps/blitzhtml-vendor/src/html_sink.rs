...
impl<'m, 'doc> DocumentHtmlParser<'m, 'doc> {
    ...
    pub fn parse_into_mutator<'a, 'd>(mutr: &'a mut DocumentMutator<'d>, html: &str) -> Vec<Cow<'static, str>> {
...
impl<'m, 'doc> TreeSink for DocumentHtmlParser<'m, 'doc> {
    ...
    fn finish(self) -> Self::Output {
        // Collect non-fatal parse errors instead of printing them to stdout
        // (the stock blitz-html 0.2.0 printed `ERROR: {error}` per error,
        // which corrupted machine-readable stdout like `--json` output).
        self.errors.take()
    }
