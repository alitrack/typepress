...
fn reorder_cjk_latin(
    mut families: Vec<parley::FontFamilyName<'static>>,
) -> Vec<parley::FontFamilyName<'static>> {
    let first_cjk = families.iter().position(|f| {
        matches!(f, parley::FontFamilyName::Named(name) if is_cjk_family(name))
    });
    let first_latin_after_cjk = first_cjk.and_then(|cjk_idx| {
        families.iter().skip(cjk_idx + 1).position(|f| {
            matches!(f, parley::FontFamilyName::Named(name) if is_latin_family(name))
...
pub(crate) fn style(
    ...
    // Reorder font stack: if a CJK or emoji font appears before a Latin font
    // in the stack, move the Latin font before it. This ensures digits and
    // Latin text use proportional-width Latin fonts instead of full-width
    // CJK glyphs or emoji-style digits.
