...
impl CacheTree for FulgurLayoutTree<'_> {
    fn cache_get(&self, node_id: NodeId, input: &taffy::tree::LayoutInput) -> Option<taffy::LayoutOutput> {
        self.doc.cache_get(node_id, input)
    }
    fn cache_store(&mut self, node_id: NodeId, input: &taffy::tree::LayoutInput, layout_output: taffy::LayoutOutput) {
        self.doc.cache_store(node_id, input, layout_output);
    }
