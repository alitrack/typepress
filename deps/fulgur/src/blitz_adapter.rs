...
pub mod net {
    ...
    pub use blitz_traits::net::{Bytes, NetProvider, Request, Url};
}
...
pub fn parse(html: &str, viewport_width: f32, font_data: &[Arc<Vec<u8>>]) -> HtmlDocument {
    parse_inner(html, viewport_width, 10000, font_data, true, None)
}
...
pub fn parse_html_with_local_resources(
    ...
    // TypePress does not use network resource loading (<link>, @import).
    // Skip FulgurNetProvider entirely to avoid the blitz-traits 0.3 API churn.
    ...
    (doc, crate::gcpm::GcpmContext::default())
}
...
fn parse_inner(
    ...
    let config = DocumentConfig {
        ...
        net_provider: Some(Arc::new(blitz_traits::net::DummyNetProvider)),
...
///
/// Stubbed: usvg 0.45/0.46 diamond dependency between krilla-svg and blitz-dom.
/// Returns None — inline SVG rendering disabled pending upstream version alignment.
pub fn extract_inline_svg_tree(
    _elem: &blitz_dom::node::ElementData,
...
///
/// Stubbed: stylo 0.18 removed `VerticalAlign` and `clone_vertical_align()`.
/// Restore when upstream re-adds the API or use a custom parser.
pub fn extract_vertical_align(_node: &blitz_dom::Node) -> crate::paragraph::VerticalAlign {
    crate::paragraph::VerticalAlign::Baseline
}
