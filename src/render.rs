...
/// Remove all inline `<style>` blocks from HTML.
///
/// When the user supplies explicit `--css` files, their stylesheets
/// (injected via the AssetBundle → InjectCssPass) must be the single
/// source of truth. Inline `<style>` blocks shipped inside the input
/// document (e.g. `pandoc --standalone` embeds its own default
/// stylesheet) conflict with the injected CSS — with equal specificity
/// the cascade order is ambiguous, and blitz-dom then mis-lays-out
/// table cells (narrow CJK columns wrap character-by-character,
/// producing one glyph per line). Returns the number of blocks removed.
fn strip_inline_styles(html: &mut String) -> usize {
    let re = regex::Regex::new(r"(?is)<style[^>]*>.*?</style>").expect("valid style regex");
    let original = std::mem::take(html);
    let removed = re.find_iter(&original).count();
    let cleaned = re.replace_all(&original, "");
    *html = cleaned.into_owned();
    removed
}
...
pub fn render_document(opts: RenderOptions) -> Result<RenderOutput> {
    ...
    if is_md {
        ...
        // 0a. When explicit --css files are supplied, strip inline <style>
        // blocks from the input document so the injected stylesheet is the
        // single source of truth. Input HTML generated by tools such as
        // `pandoc --standalone` embeds its own default stylesheet, which
        // conflicts with the injected CSS: with equal specificity the
        // cascade order is ambiguous and blitz-dom then mis-lays-out
        // table cells (narrow CJK columns wrap character-by-character).
        if !opts.css_files.is_empty() {
            let stripped = strip_inline_styles(&mut html);
            if stripped > 0 {
                eprintln!("Stripped {stripped} inline <style> block(s) (explicit --css supplied)");
            }
        }
